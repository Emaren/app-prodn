...
const MATCH_ID = 23;
const EXPECTED_WAGER_WOLO = 25;
const EXPECTED_GUARANTEE_WOLO = 10;
const EXPECTED_REFUND_WOLO = 35;
const CONFIRMATION = "EMAREN-JIM-23-35";
...
function assertEmarenJimPlan(plan: ScheduledMatchSettlementPlan) {
  const emaren = participantByName(plan, "emaren");
  const jim = participantByName(plan, "jim");
  if (!emaren || !jim || emaren.side === jim.side) {
    throw new Error(
      `STOP: challenge #${MATCH_ID} no longer resolves uniquely to Emaren vs Jim (title: ${plan.title}).`
    );
  }
  ...
  if (
    ...
    throw new Error(
      `STOP: challenge #${MATCH_ID} terms changed. Expected 25 wager + 10 guarantee = 35 WOLO; got ${plan.terms.wagerAmountWolo} + ${plan.terms.guaranteeAmountWolo} = ${plan.terms.totalFundingWolo}.`
    );
  }

  if (!["canceled", "cancelled", "expired"].includes(normalized(plan.status))) {
    throw new Error(
      `STOP: challenge #${MATCH_ID} status is ${plan.status}; expected cancelled/expired.`
    );
  }

  if (plan.liability.fundedParticipantCount !== 1) {
    throw new Error(
      `STOP: challenge #${MATCH_ID} has ${plan.liability.fundedParticipantCount} funded participants; expected exactly one.`
    );
  }

  if (!emaren.funded || jim.funded) {
    throw new Error(
      `STOP: funding truth changed. Expected Emaren funded=true and Jim funded=false; got Emaren=${emaren.funded}, Jim=${jim.funded}.`
    );
  }
  ...
  if (
    ...
    throw new Error(
      `STOP: liability mismatch. Expected exactly 35 WOLO refund and 0 treasury; got funded=${plan.liability.fundedLiabilityWolo}, planned=${plan.liability.plannedTransferWolo}, refund=${plan.liability.refundWolo}, treasury=${plan.liability.treasuryWolo}.`
    );
  }
  ...
  if (
    ...
    normalized(transfer.participantName) !== normalized(emaren.name) ||
    !transfer.recipientAddress
  ) {
    throw new Error(
      `STOP: refund transfer is not the exact Emaren 35 WOLO return expected for challenge #${MATCH_ID}.`
    );
  }

  return { emaren, jim, transfer };
}
...
try {
  ...
  const { transfer } = assertEmarenJimPlan(before);
  printPlan("EMAREN / JIM #23 PRE-FLIGHT", before);
  ...
  if (before.state === "executed") {
    ...
    if (before.blockers.length > 0) {
      throw new Error(`STOP: challenge #${MATCH_ID} blockers: ${before.blockers.join(" | ")}`);
    }
    if (!before.dryRun?.ok) {
      throw new Error(
        `STOP: escrow dry-run failed: ${before.dryRun?.detail || before.dryRun?.failureCode || "unknown"}`
      );
    }
    console.log("\nPASS: exact 35 WOLO Emaren refund is ready and dry-run verified.");
    console.log(`Execute once with: npm run challenge:emaren23 -- --execute --confirm=${CONFIRMATION}`);
  } else {
    if (confirm !== CONFIRMATION) {
      throw new Error(`STOP: execution requires --confirm=${CONFIRMATION}. No funds moved.`);
    }
    if (
      (before.state !== "ready" && before.state !== "failed") ||
      before.blockers.length > 0 ||
      !before.dryRun?.ok
    ) {
      throw new Error("STOP: exact escrow dry-run is not green. No funds moved.");
    }

    console.log("\n== EXECUTING EXACTLY ONE IDEMPOTENT EMAREN REFUND ==");
    const result = await executeScheduledMatchSettlement(prisma, MATCH_ID, null);
    console.log(JSON.stringify({ ok: result.ok, execution: result.execution }, null, 2));

    const after = await readExactPlan(false);
    const afterTruth = assertEmarenJimPlan(after);
    printPlan("EMAREN / JIM #23 POST-FLIGHT", after);
    if (
      after.state !== "executed" ||
      afterTruth.transfer.existingSettlement?.status !== "executed" ||
      !afterTruth.transfer.existingSettlement.txHash
    ) {
      throw new Error("STOP: post-flight refund proof is incomplete.");
    }
    console.log(`\nPASS: challenge #${MATCH_ID} refunded exactly 35 WOLO to Emaren.`);
