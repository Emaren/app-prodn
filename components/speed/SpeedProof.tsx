...
export default function SpeedProof() {
  ...
  useEffect(() => {
    ...
    if (!AUTHORITATIVE_ROUTES.has(route)) return;
  ...
  return (
    ...
    >
      {open && sample && duration ? (
        ...
        >
          ...
          <div className="mt-1.5 text-sm font-semibold text-white">
            {restored ? "Restored" : "Ready"} in {duration}
          </div>
      ...
      <button
        ...
        className={`flex h-9 items-center gap-1.5 rounded-full border px-3 text-xs shadow-[0_10px_30px_rgba(0,0,0,0.32)] backdrop-blur-xl transition focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-200/40 ${
          duration
            ? "cursor-pointer border-amber-100/15 bg-[#090b11]/88 text-amber-50/90 hover:border-amber-100/25 hover:bg-[#0d1018]/94"
            : "cursor-default border-white/8 bg-[#090b11]/72 text-white/45"
        }`}
        aria-expanded={duration ? open : undefined}
        aria-label={
          duration
            ? `AoE2WAR Speed Proof: ${restored ? "restored" : "ready"} in ${duration}`
      ...
      >
        ...
        {duration ? (
          <span>
            {restored ? "Restored" : "Ready"} in <span className="font-semibold text-white">{duration}</span>
          </span>
        ) : (
          <span>Measuring…</span>
        )}
