...
function MiniPreview({ mode }: { mode: string }) {
  return (
    <span
...
export default function ClanChatViewPicker({
  ...
  controlledMode?: ClanChatViewMode;
  onControlledChange?: (mode: ClanChatViewMode) => void;
  labelPrefix?: string;
}) {
  const {
    chatViewMode: viewerMode,
    setChatViewMode: setViewerMode,
  } = useClanChatViewPreference({
    clanSlug,
    defaultMode,
  });
  const chatViewMode = controlledMode ?? viewerMode;
  const setChatViewMode = onControlledChange ?? setViewerMode;
  ...
  const active =
    CLAN_CHAT_VIEWS.find((view) => view.key === chatViewMode) ??
    CLAN_CHAT_VIEWS[0];
  const activeIndex = Math.max(
    0,
    CLAN_CHAT_VIEWS.findIndex((view) => view.key === chatViewMode),
  );

  function cycleChatView() {
    const next = CLAN_CHAT_VIEWS[(activeIndex + 1) % CLAN_CHAT_VIEWS.length];
  ...
  return (
    ...
    >
      <button
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        aria-label={`${labelPrefix} ${active.version} ${active.label}. Click for next view; hover to choose any view.`}
        title={`${labelPrefix} · ${active.version} · ${active.label} · Click for next view · Hover to choose`}
      >
      ...
      {open ? (
        <div
          ...
          role="menu"
          aria-label={`${labelPrefix} versions`}
        >
          {CLAN_CHAT_VIEWS.map((view) => {
            const selected = view.key === chatViewMode;
            return (
              <button
                key={view.key}
                type="button"
                ...
                className={`clan-chat-view-option${
                  selected ? " clan-chat-view-option--active" : ""
                }`}
                onClick={() => {
                  cancelFanClose();
                  setChatViewMode(view.key);
                  setOpen(false);
                }}
              >
                <MiniPreview mode={view.key} />
