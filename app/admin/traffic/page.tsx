...
function StatCard({
  ...
  return (
    <div
      className={`rounded-2xl border border-slate-800 bg-slate-950/80 p-4 ${className}`}
    >
      <p className="text-sm leading-5 text-slate-400">{label}</p>
      <p className={`mt-2 text-2xl font-semibold ${accentClass}`}>{value}</p>
      {sublabel ? (
        <p className="mt-2 text-xs leading-5 text-slate-500">{sublabel}</p>
      ) : null}
    </div>
  );
}
    ...
    <ul className="space-y-2">
      {rows.map((row) => (
        ...
        >
          ...
          <span className={`shrink-0 font-semibold ${accentClass}`}>
      ...
      {hosts.map(([host, rows]) => (
        ...
        >
          <div className="mb-3 flex items-center justify-between gap-3">
            <h3 className="min-w-0 flex-1 truncate font-semibold text-slate-100">
              {host}
            </h3>
...
export default function TrafficPage() {
  ...
  const liveIps: IpRow[] = useMemo(
    () => asIpRows(summary.top_ips_24h),
    [summary.top_ips_24h],
  );
  ...
  const humanCountries: CountRow[] = useMemo(() => {
    ...
    return (summary.top_countries || []).map(([label, count]) => ({
      label,
      count,
    }));
  ...
  const suspiciousPaths: CountRow[] = asCountRows(
    summary.top_suspicious_paths_24h,
  );
  ...
  const categoryRequestCounts: CountRow[] = asCountRows(
    summary.category_request_counts_24h,
  );

  const topHosts: CountRow[] = asCountRows(summary.top_hosts_24h);
  const topHumanHosts: CountRow[] = asCountRows(
    summary.host_unique_human_ips_24h,
  );
  const topSuspiciousHosts: CountRow[] = asCountRows(
    summary.host_unique_suspicious_ips_24h,
  );

  const topPathsByHost: HostCounterMap = asHostCounterMap(
    summary.top_paths_by_host_24h,
  );
  const topSuspiciousPathsByHost: HostCounterMap = asHostCounterMap(
    summary.top_suspicious_paths_by_host_24h,
  );
  const topReferrersByHost: HostCounterMap = asHostCounterMap(
    summary.top_referrers_by_host_24h,
  );
  ...
  const primaryAliases =
    Array.isArray(summary.primary_host_aliases) &&
    summary.primary_host_aliases.length > 0
      ? summary.primary_host_aliases
      : SUMMARY_DEFAULTS.primary_host_aliases!;

  const profileGapUids = useMemo(
    () => asStringArray(stats?.profile_gap_uids),
    [stats?.profile_gap_uids],
  );
      ...
      <div className="min-h-screen bg-[#050816] p-6 text-white">
        <div className="mx-auto max-w-5xl rounded-2xl border border-slate-800 bg-slate-950/80 p-6">
          ...
          <p className="mt-3 text-slate-300">
            Loading traffic command center...
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#050816] text-white">
      <div className="mx-auto max-w-7xl space-y-6 p-4 md:p-6">
        <header className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
              Unified VPS traffic across hosted projects. This page now
              understands hosts too, so you can see which domains are active,
              which ones are getting real people, and which ones are drawing
              probes and trash.
            </p>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-950/80 px-4 py-3 text-sm text-slate-300">
            ...
            <p className="mt-1">
              ...
              <span className="font-medium text-slate-100">
                {formatTimestamp(summary.last_log_time)}
              </span>
            </p>
          </div>
        </header>
        ...
        <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/10 px-4 py-3 text-sm leading-6 text-cyan-100">
          This dashboard is intentionally broad right now. It shows combined
          traffic observed by the VPS across your hosted projects so you can map
          the whole battlefield before splitting this into project-specific
          dashboards later.
        </div>
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          <Panel
            title="Top hosts (24h)"
            subtitle="Highest-volume domains in the current parsed window."
          >
            <HostList
              rows={topHosts}
              accentClass="text-sky-300"
              emptyText="No host data yet."
            />
          </Panel>
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          <Panel
            title="Live IP activity (24h)"
            subtitle="The busiest IPs in the current parsed window."
          >
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-800 text-sm">
                  ...
                  <tbody className="divide-y divide-slate-900">
                    {(showMoreLiveIps ? liveIps : liveIps.slice(0, 8)).map(
                      (row) => (
          ...
          >
            ...
            {allTimeIps.length > 8 ? (
              ...
              >
                {showMoreAllTimeIps
                  ? "Show fewer all-time IPs"
                  : "Show more all-time IPs"}
              </button>
            ) : null}
          </Panel>
        </div>
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          <Panel
            title="Top countries"
            subtitle="Human-like countries first, then all recent activity."
          >
            <div className="space-y-4">
              <div>
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {allCountries
                    .slice(0, showMoreCountries ? 10 : 4)
                    .map((row) => (
                      <li
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          <Panel
            title="Top paths by host"
            subtitle="What each ranked host is actually serving."
          >
            <HostMapPanel
              data={topPathsByHost}
          ...
          >
            <ul className="space-y-2">
              {suspiciousPaths.slice(0, 10).map((row) => (
                ...
                >
                  <span className="min-w-0 flex-1 truncate font-mono text-rose-100">
                    {row.label}
                  </span>
        ...
        >
          ...
          <div className="mb-3 rounded-xl border border-cyan-500/20 bg-cyan-500/5 px-3 py-2 text-xs text-cyan-200">
            Canary check: if you can see{" "}
            <span className="font-semibold">{BUILD_MARKER}</span> on the live
            page, you are definitely looking at the updated{" "}
            <code className="mx-1">app/traffic/page.tsx</code>.
          </div>
