...
const BOARD_ROWS: Record<
  BoardKey,
  Array<{ player: string; staked: string; rewards: string; status: string }>
> = {
  stakers: [
    { player: "Founder", staked: "--", rewards: "--", status: "Founding slot" },
    { player: "Early Staker", staked: "--", rewards: "--", status: "Ready soon" },
    { player: "Match Regular", staked: "--", rewards: "--", status: "Waiting" },
    { player: "New Backer", staked: "--", rewards: "--", status: "Open seat" },
  ],
  earners: [
    { player: "Founder", staked: "--", rewards: "--", status: "Preview" },
    { player: "Early Staker", staked: "--", rewards: "--", status: "Preview" },
    { player: "Match Regular", staked: "--", rewards: "--", status: "Preview" },
    { player: "New Backer", staked: "--", rewards: "--", status: "Preview" },
  ],
  rewards: [
    { player: "Daily Pool", staked: "--", rewards: "--", status: "Preparing" },
    { player: "Fee Split", staked: "0.75%", rewards: "50%", status: "Stakers" },
    { player: "Treasury", staked: "0.75%", rewards: "50%", status: "Community" },
    { player: "Next Match", staked: "--", rewards: "--", status: "Feeds pool" },
  ],
};
...
function formatWolo(value: number | null) {
  if (value == null) return "--";
  return `${formatCompact(value)} WOLO`;
}
...
async function loadEconomySnapshot(period: PeriodKey): Promise<EconomySnapshot> {
  ...
  const bettingFeePool = Math.round(settledVolumeWolo * BETTING_FEE_RATE);
  const stakerFeePoolWolo = Math.round(bettingFeePool * STAKER_SHARE);
  const treasuryShareWolo = bettingFeePool - stakerFeePoolWolo;

  const activity: ActivityItem[] = [];

  if (stakerFeePoolWolo > 0) {
    activity.push({
      label: `${formatWolo(stakerFeePoolWolo)} modeled for stakers`,
      detail: "50% of the betting fee from settled volume.",
      meta: "Fee model",
      tone: "amber",
    });
  }
  ...
  if (treasuryShareWolo > 0) {
    activity.push({
      label: `${formatWolo(treasuryShareWolo)} modeled for treasury`,
      detail: "Community share from the same betting fee.",
      meta: "Fee model",
      tone: "emerald",
    });
  }
  ...
  for (const wager of recentWagers) {
    ...
    activity.push({
      label: isWin
        ? `${player} won ${formatWolo(wager.payoutWolo ?? 0)}`
        : `${player} backed ${pickedLabel}`,
      detail: `${formatWolo(wager.amountWolo)} on ${wager.market.title}`,
    ...
    <main className="space-y-6 py-3 text-white sm:space-y-7 sm:py-4">
        ...
        <div className="relative z-10 grid gap-7 xl:grid-cols-[1.02fr_0.98fr] xl:items-start">
          <div className="space-y-6">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <HeroStat label="Reward Weight" value="WOLO x time" helper="Fair daily share" />
          ...
          <section className="rounded-[1.65rem] border border-white/10 bg-[linear-gradient(180deg,rgba(7,12,22,0.94),rgba(3,6,12,0.98))] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:p-6">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              ...
              <HeroStat label="Staker Pool" value={formatWolo(snapshot.stakerFeePoolWolo)} helper="Modeled fee share" featured />
              <HeroStat label="Treasury Share" value={formatWolo(snapshot.treasuryShareWolo)} helper="Modeled fee share" />
            </div>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <EconomyCard
            ...
            label="Staker Pool"
            value={formatWolo(snapshot.stakerFeePoolWolo)}
            helper="50% betting-fee share"
            tone="amber"
            featured
          />
          <EconomyCard
          ...
          <EconomyCard
            ...
            value={formatWolo(snapshot.treasuryShareWolo)}
            helper="50% betting-fee share"
            tone="amber"
          />
          <EconomyCard
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        ...
        <RewardCard
          ...
          title="Leave Anytime"
          copy="Unstake whenever you need your WOLO. Rewards stay fair because they are based on amount x time."
        />
      </section>
      ...
      <section className="rounded-[1.65rem] border border-white/10 bg-[linear-gradient(180deg,rgba(10,16,29,0.92),rgba(4,7,14,0.98))] p-5 sm:p-6">
        <div className="grid gap-5 lg:grid-cols-[0.82fr_1.18fr]">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <FormulaTile label="Your Weight" value="WOLO x time" />
      ...
      <section className="grid gap-4 lg:grid-cols-5">
        ...
        <TrustCard title="Fair Weight" copy="Reward share is based on WOLO x time." />
      ...
      <section className="overflow-hidden rounded-[1.65rem] border border-amber-300/18 bg-[radial-gradient(circle_at_16%_18%,rgba(251,191,36,0.18),transparent_30%),linear-gradient(135deg,rgba(18,24,38,0.98),rgba(6,10,18,0.98))] p-6 sm:p-8">
        <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-semibold text-white">Ready to Put WOLO to Work?</h2>
...
function LeaderboardRow({
  ...
  rank: number;
  row: { player: string; staked: string; rewards: string; status: string };
}) {
  return (
    <div className="rounded-[1.1rem] border border-white/10 bg-white/[0.04] p-4 md:grid md:grid-cols-[3rem_1.3fr_1fr_1fr_0.9fr] md:items-center md:gap-3">
      ...
      <div className="mt-3 font-semibold text-white md:mt-0">{row.player}</div>
      <MobileLabel label="Staked" value={row.staked} />
      <MobileLabel label="Rewards" value={row.rewards} />
      <div className="mt-3 rounded-full border border-emerald-300/15 bg-emerald-500/10 px-3 py-1 text-xs text-emerald-100 md:mt-0 md:text-center">
        {row.status}
      </div>
    </div>
  );
