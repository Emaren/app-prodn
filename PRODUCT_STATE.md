---
id: "aoe2war.app-prodn.product-state"
title: "app-prodn Product State"
type: "historical"
status: "historical"
owner: "aoe2war-web"
...
Production was inspected read-only against the July 26 source snapshot and the live VPS.

- web checkout: `/var/www/AoE2HDBets/app-prodn`, branch `main`, clean at `22232a0bcc038a567acd052f432883e70482a3f9`, equal to `origin/main`;
- deployed web build: `20260726054351-9b5a6fcd0b`, healthy locally and publicly on port `3030`;
- replay API checkout: `/var/www/AoE2HDBets/api-prodn`, branch `main`, clean at `d2d68646b1aff3ffb9e647ee0fe4deaa143b2c6e`, healthy on port `3330`;
- active WoloChain checkout: `/var/www/WoloChain-wolo-1`, branch `wolo-1-mainnet-prep`, clean at `d5dea8d6f1a2b0b57489a5e468dd21e34246891e`, equal to its remote branch;
- Watcher release manifests and Windows/macOS/Linux artifacts advertise `1.5.6` and match their staged release hashes;
- PostgreSQL contains 73 migration-history rows: 71 successfully applied, 0 incomplete, and 2 historical rolled-back attempts; Prisma reports all 71 source migrations up to date;
- the six July 22–26 replay, roster, normalized-stat, financial-authority, and post-broadcast recovery migrations are applied;
- Wolo mainnet, REST/RPC, Bet settlement (`8092`), Founder Rewards settlement (`8093`), transfer heartbeat, challenge reconciliation, staking, and trophy rails were healthy at inspection.

Production parity does not mean every operational rail is healthy. At inspection, the replay auto-recovery timer was `active (elapsed)` with no next trigger and root storage was 94% used. Post-seal remediation repaired the timer, reclaimed root space to 3.33 GiB free, recovered the eligible replay candidate successfully, and verified a subsequent recurring timer run with `Result=success`. Root capacity remains a P1 risk because it is still below the preferred 6 GiB operating floor. API/settlement service hardening also remains weaker than the web service. See `docs/PRODUCTION_PARITY_2026-07-26.md`.


## Watcher and realtime reliability

Watcher v1.5.7 separates app connection from monitor attachment, validates/scans HD folders (including OneDrive), recovers mid-game files, binds upload metadata and bodies to one immutable byte snapshot, reports rich privacy-safe heartbeat state, and uses bounded watchdog reattachment. `/bets` removes detached books from active liquidity, shows them under Awaiting Final Proof without controls, persists a stable proof deadline for every entry path, repairs historical null clocks with one fresh bounded migration grace, voids after grace when proof never arrives, and preserves honest refund state/history. Metadata-only HD recovery rows yield to substantive roster iterations, so coherent live propositions no longer wait for the full session-freshness window. Required on-chain staking pauses when settlement health cannot be verified.

The betting board refreshes at two seconds while visible and on foreground resume. The live-games board keeps its five-second cadence and foreground refresh, while expired shared snapshots coalesce on fresh DB truth instead of returning a healthy-path stale-while-revalidate result.
