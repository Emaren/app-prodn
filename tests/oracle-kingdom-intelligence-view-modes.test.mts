...
test("Oracle preserves Basic while Kingdom Intelligence defaults Advanced and both keep B/A/E width provenance", () => {
  ...
  assert.match(premiumOracle, /data-oracle-advanced-hero="workshop-advanced-exact"/);
  ...
  assert.match(
    premiumOracle,
    /data-oracle-extreme-banner="workshop-advanced-exact"[\s\S]*?className="object-cover object-center"/,
  );
  assert.doesNotMatch(
    premiumOracle,
    /data-oracle-extreme-banner="workshop-advanced-exact"[\s\S]*?scale-\[1\.04\]/,
  );
  assert.match(
    premiumOracle,
