...
- The global command bar orders the Universal Translator’s fading language signal and wireframe globe before NavChat and the player control; Auto cycles quietly, explicit choices persist locally and in a cookie, and only `/academy` uses the crimson selector while the rest of the product uses navy/steel blue
...
- `/live-games` persists its own Basic / Advanced / Extreme mode, defaults to the full-width Extreme board, and limits the just-finished spotlight to three outcomes before older results flow into Recently Played.
