...
- `/live-games` persists its own Basic / Advanced / Extreme mode and defaults to the original Basic Now Playing shell. The Basic live-session card keeps watcher coverage, stream preview, Watch Theatre, live stats, lobby, and Bet Rail actions; Advanced and Extreme remain available behind the low-key View control. The just-finished spotlight is limited to three outcomes before older results flow into Recently Played.
...
- Watcher-native video is expendable under replay pressure. The server can terminal-shed watcher-native chunks before reading their bodies while an app-proxied replay upload is in flight **when the watcher advertises `server-media-shed-v1`**; `AOE2_STREAM_MEDIA_KILL_SWITCH=1` forces the same video-only shed path for an operator emergency. Replay/API traffic remains authoritative.
...
- watcher sessions do not become bettable until both sides have confident parsed labels. Placeholder labels such as `Players parsing`, `Live 4v4`, `Unknown`, and `Game in progress` are rejected at seed, sync, and response time; zero-wager/zero-intent/zero-seed placeholders are archived without touching markets that hold money.
