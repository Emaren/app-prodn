...
export const CLAN_CREST_KIND = "crest";
...
export function clanCrestPoolTarget(slug: string) {
  const target = normalizeManagedMediaTarget(
    `clan-${slug}-crest-pool`,
  );

  if (!target) {
    throw new Error("Could not build clan crest pool target.");
  }
...
export function clanCurrentCrestTarget(slug: string) {
  ...
  if (!target) {
    throw new Error("Could not build current clan crest target.");
  }
