...
function pythonTestRatio(commands: Map<string, Json>): InspectionRatio | null {
  ...
  const match = output.match(
    /Running\s+(\d+)\s+active Python contract files;\s+(\d+)\s+explicitly quarantined/i,
  );
  if (!match) return null;
  const active = Number(match[1]);
  const quarantined = Number(match[2]);
  return {
    passed: commandPassed(commands, "active-python-test-contract") ? active : 0,
    total: active + quarantined,
  };
}
  ...
  const tests = category(
    ...
    [
      ...
      check(
        ...
        pythonRatio
          ? pythonRatio.passed + "/" + pythonRatio.total + " active files"
