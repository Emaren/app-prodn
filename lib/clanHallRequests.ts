...
export function parseClanHallRequestText(
  ...
  if (!text || !text.startsWith(`${CLAN_HALL_REQUEST_MARKER}\n`)) return null;
  ...
  const desiredSlug = slugifyClanHallName(readField(metadata, "Requested slug:"));
  const foundingMessage = text.slice(separatorIndex + separator.length).trim();

  if (!clanName || !desiredSlug || !foundingMessage) return null;
  return { clanName, desiredSlug, foundingMessage };
}

export function buildClanAlertMessage(input: {
  requester: string;
  amountWolo: number;
  requestId: string;
  payment: string;
  ...
  return [
    ...
    `Clan: ${input.details.clanName}`,
    `Requested hall: /clans/${input.details.desiredSlug}`,
    `Purchase: ${input.amountWolo} WOLO verified`,
    `Request ID: ${input.requestId}`,
    `Payment: ${input.payment}`,
    "---",
    input.details.foundingMessage,
  ].join("\n");
}
