...
## Finish documentation tree-identity proof

The 2026-09-18 release audit measured documentation/context work at 5.41 minutes,
67.5% of an 8.01-minute Finish. Central documentation synchronization was
running docs-check, taxonomy audit, and strict MkDocs build both before and
after committing the same generated bytes. The durable contract is now:
validate the generated staged tree once, capture its Git tree object with
`git write-tree`, commit/push, then require the committed `HEAD^{tree}` to
match exactly and require a clean index/worktree. Git tree identity makes the
